	    ...
	    <div class="row">
	      <div class="col s12 offset-m3 m6">
	        ...
	        <form id="UserForm" name="UserForm" method="post"  >
				...
				<input type="hidden" name="new_pwd" id="new_pwd" value="" />
				<input type="hidden" name="old_pwd" id="old_pwd" value="" />
				
				<div class="user-info-write">
					<dl>
						<dt style="width: 28%;">기존 비밀번호</dt>
						<dd><input type="password" class="kt-input" title="기존 비밀번호" id="ipt_old_pwd"  autocomplete="off"></dd>
					    </dl>
					<dl>
						<dt style="width: 28%;">신규 비밀번호</dt>
						<dd><input type="password" class="kt-input" title="신규 비밀번호 " id="ipt_new_pwd" autocomplete="off"></dd>
					    </dl>
					<dl>
						<dt style="width: 28%;">신규 비밀번호 확인</dt>
						<dd><input type="password" class="kt-input" title="신규 비밀번호 확인 " id="ipt_new_c_pwd" onkeydown="if(event.keyCode == 13) {return checkMsg();}" autocomplete="off"></dd>
					</dl>
				    <dl>
				        <dt></dt>
				        <dd>
...
function checkMsg(){
  ...
  //<!-- 2017.12.22  보안성검토 비번 체크로직 추가 -->
    if ($("#ipt_new_pwd").val().indexOf(" ") != -1){
        alert("비밀번호에 공백이 있습니다. 다시 입력해 주십시오.");
        return false;
    }
  //<!-- 2017.12.22  보안성검토 비번 체크로직 추가 -->
    //비밀번호 동일 문자 연속문자 체크
    if(!validate_samepass($("#ipt_new_pwd").val())) {                    
           document.UserForm.new_pwd.focus();
           document.UserForm.new_pwd.select();
           return false;
       }
  //<!-- 2017.12.22  보안성검토 비번 체크로직 추가 -->
    //ID와 PW비교하여 ID에서 연속되는 문자 4개 있으면 리턴
     if(!validate_samepass2($("#ipt_new_pwd").val(),document.UserForm.elements["user_id"].value)) {                    
        document.UserForm.new_pwd.focus();
        document.UserForm.new_pwd.select();
        return false;
    } 
    //비밀번호 영문자, 숫자, 특수문자 조합    
    if(!validate_password($("#ipt_new_pwd").val())) {
    ...
    //사용자ID 랑 비밀번호 체크
    if(document.UserForm.elements["user_id"].value == $("#ipt_new_pwd").val()) {
   ...
   //기존 비번이랑 신규 비번이랑 동일 할 경우
    if($("#ipt_old_pwd").val() == $("#ipt_new_pwd").val()) {
    ...
    if($("#ipt_new_pwd").val() != $("#ipt_new_c_pwd").val()) {
	...
	// 전송용으로 추가한 input hidden에 값 추가 (이때 RSA로 암호화)
	document.UserForm.elements["new_pwd"].value = rsa.encrypt($("#ipt_new_pwd").val());
	document.UserForm.elements["old_pwd"].value = rsa.encrypt($("#ipt_old_pwd").val());
